...
# wrangler refresh tokens are single use: keep HER refreshed login before putting yours back
trap 'cp "$CFG/default.toml" "$CFG/default.toml.smile-client" 2>/dev/null || true; cp "$PARK" "$CFG/default.toml" 2>/dev/null || true' EXIT
